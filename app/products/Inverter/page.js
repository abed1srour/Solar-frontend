  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        // Filter for Inverter category
        const inverters = allProducts.filter(product => product.category === 'Inverter');
        setProducts(inverters);
        setError(null);
      } catch (err) {
    ...
    <div className="py-8 px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-white">Inverters</h1>
            <p className="text-gray-400 mt-1">High-quality inverters for your solar power system</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-gray-800 rounded-xl border border-gray-700">
            <p className="text-gray-400 text-lg">No inverters found in our inventory.</p>
          </div>
        ) : (
