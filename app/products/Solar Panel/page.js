...
export default function SolarPanelProducts() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        // Filter for Solar Panel category
        const solarPanels = allProducts.filter(product => product.category === 'Panel');
        setProducts(solarPanels);
        setError(null);
      } catch (err) {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-400 mt-1">High-efficiency solar panels for your renewable energy needs</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-gray-800 rounded-xl border border-gray-700">
            <p className="text-gray-400 text-lg">No solar panels found in our inventory.</p>
          </div>
        ) : (
